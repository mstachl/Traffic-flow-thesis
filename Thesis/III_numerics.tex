...
\newline
\newline
The \textbf{Godunov method} now consists of seperately solving every Riemann problem between every two neighboring cells. Let $u_l$ and $u_r$ be the densities on two neighboring cells. Then the flux $f(u^*(u_l,u_r)$ is given by
\begin{equation}
f(u^*(u_l,u_r))=\begin{cases} \min [f(u_l),f(u_r)] \hspace{1cm}\text{if} \ u_l\leq u_r \\
f(\sigma)\hspace{1cm}\text{if} \ u_r\leq\sigma\leq u_l \\
f(u_r) \hspace{1cm}\text{if} \ \sigma\leq u_r \leq u_l \ \text{ or } u_r\leq u_l\leq\sigma\end{cases}
\end{equation}
\textcolor{red}{ADD picture of different shocks, rarefaction waves} \newline
\newline
In the following we want to apply the Godunov scheme on the buffer model defined in section \ref{buffer}
\subsection{The Godunov scheme on networks}
...
\end{equation} 
with the inflowing density $v_{i,1}^n:=\int_{t_n}^{t_{n+1}}q_i(t)dt$. \newline
The outfluxes leaving the network can be treated analogously. \newline
\newline
For junctions we recall Definiton \ref{SBJ} for the junction fluxes in the single buffer model. Accordingly, we define for roads coming into junction $J$
\begin{equation}
