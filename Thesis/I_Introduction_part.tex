...
\begin{itemize}
	...
	\item \textbf{Mesoscopic} - or kinetic - models analyze transportation elements in small homogenous groups, where the probability of each group to be at time $t$ at location $x$ with velocity $v$ can be described by a function $f(t,x,v)$. The family of mesoscopic models includes headway distribution models \citep{Buckley.1968} and cluster models \citep{Mahnke.2007}. This modelling approach often requires the use of methods from statistical mechanics.
...
\end{equation}
For conserved quantities no mass is neither destroyed nor created. Therefore the mass on an interval can only change by inflowing and outflowing quantities. The quantity mass crossing the point $x$ at a time $t$ is given by the \textbf{traffic flux} $f:[0,\rho_{max}]\rightarrow[0,f^{max}]$, where $\rho_{max}$ denotes the maximum car density on the network. \newline 
\begin{figure}[h]
\center
...
\end{figure}
\newline
The flux $f$, by assumption, is a Lipschitz continous function satisfying $f(0)=f(\rho_{max})=0$.
We also assume that the traffic flux, or also referred to as \textit{fundamental diagram}, follows the rule
\begin{equation*}
\label{eq:flux_condition}
f'(\rho)\left(\sigma-\rho\right)>0
\end{equation*}
where 
\begin{equation}\label{eq:max_flux}
\sigma := \text{arg}\max_\rho f(\rho)
\end{equation}
denotes the density where the maximum flux \begin{equation*}
f^{max} :=  f(\sigma)
\end{equation*}
is attained. This value is usualy referred to as \textit{road capacity}. \newline
Since the total mass on the global space interval is preserved, the rate of change of the mass on the interval $\left[x_1,x_2\right]$ is defined by the difference of the in- and outfluxes of the interval (assuming that $\rho(x,\cdot)\in L^1_{loc}$, e.g. \begin{eqnarray*}
