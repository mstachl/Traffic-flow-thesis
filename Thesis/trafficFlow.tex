...
where $u: \mathbb{R}\times\mathbb{R}\rightarrow\mathbb{R}^n$ denotes $n$ conserved quantities or state variables, e.g. densities or energy levels. In particular $u(x,t)$ represents the distribution of the conserved quantity on the space interval at every time step t. Furthermore, the total mass of an observed quantity on a space interval $\left[x_1,x_2\right]$ is given by $\label{eq:mass}\int_{x_1}^{x_2} u(x,t)dx$ at a fixed time $t$. Knowing the mass distribution $u(x,t)$ at a certain time step, the specific movement of the quantities in space dimension is given by the so-called \textbf{flux function} $f:\mathbb{R}^n\rightarrow\mathbb{R}^n$. Typically the flux function is not linear in $u$, which results in a system of non-linear partial differential equations (for an example, see \citep{LeVeque.1992})
...
TODO:
\begin{itemize}
\item ...computes by its nature piecewise constant functions
...
\begin{Definition}
\label{network}
A network is a tuple $\left(\mathcal{N},\mathcal{E}\right)$, where $\mathcal{N}$ is a finite collection of vertices, and $\mathcal{E}$ a finite collection of $M$ edges, where every node represents a junction and every edge represents a unidirectional road. Each road $e_i=\left[a_i,b_i\right]\in\mathbb{R}, i=1...N$ is defined over a real interval. 
\end{Definition}
...
\begin{equation}
\label{eq:queue}
\dot{q_j}=\sum_{i\in\mathcal{I}}\bar{\Theta}_{ij}\bar{f_i}-\bar{f_j}
\end{equation}
...
\end{itemize}
\textbf{Cumulative traffic flux:} (cf. \citep{gottlich2014})
...
In order to model traffic lights in a realistic manner, we adjust the computation of the incoming fluxes \ref{eq:sbj} regarding the priority values $\eta_i$. 
\begin{Definition}[SBJ with traffic lights]
...
We then require that the incoming fluxes $\bar{f}_i$ satisfy
\[\label{eq:sbjupd}\bar{f}_i=\min\lbrace \omega_i,c_i(M-\sum_{j\in\mathcal{O}}q_j)\rbrace\cdot \eta_i, \hspace{1cm} i\in\mathcal{I}\]
The outgoing fluxes stay as in \ref{eq:sbj}.
\end{Definition}
TODO:
\begin{itemize}
\item mass conservation proof
\item MBJ
\item ...
\end{itemize}
\subsubsection{Flux optimization by partial outer convexification}
cf. \citep{gottlich2014} \\
A straightforward assumption is that an optimal traffic light control maximizes the cumulated total flux on the network. In a realistic setting also other combinatorial constraints have to be respected, e.g. maximum red phase durations or further logical constraints.
\\
In general the flux-maximizing optimization problem can be formulated as
\begin{eqnarray*}
\max_{\eta_i:\left[0,T\right]\rightarrow\left[0,1\right], i\in\mathcal{E}\setminus\mathcal{E}^{out}} &\sum_{i\in\mathcal{E}}\int_0^T\int_{a_i}^{b_i} f(\rho_i(x,t))dxdt +\sum_{i\in\mathcal{E}\setminus\mathcal{E}^{out}}\int_0^T\bar{f}_i(t)\cdot \eta^s_i(t)dt\\ &+\sum_{j\in\mathcal{E}\setminus\mathcal{E}^{in}}\int_0^T\bar{f}_j(t) dt-\gamma\int_0^T||\dot{\eta^s}||^2_1
\end{eqnarray*}
s.t. \begin{eqnarray}
  \text{conservation law}& \ref{conslaw} \\
  \text{queue}& \ref{eq:queue} \\
  \text{inital densities and queue lengths}&  \\
  \text{external inflow}&  \\
  \text{transition matrices at junctions}&  Q_{ij,J}\\
  \text{junction fluxes}&  \ref{eq:sbj}\\
  \text{valid traffic light configurations}&  \eta^s\in\Omega
\end{eqnarray}
where $||x||_1:\sum_i x_i$ denotes the 1-norm and the $\bar{f}_i$ and $\bar{f}_j$ as defined in \ref{eq:sbj}. The factor $\gamma$ weighs the cost of switching traffic lights. Also incoming and outgoing roads are denoted as in \ref{edges}
\newline
\newline
set of all possible discrete traffic light configurations: $\Omega = \left\lbrace \eta^1,...,\eta^n\right\rbrace$, where $\eta^i\in\left[0,1\right]^{|\mathcal{E}\setminus\mathcal{E}^{out}|}$ and $\eta_j^i\in\left\lbrace 0,1\right\rbrace$ the setting of the j-th road in the i-th configuration
\newline
\newline
\textbf{Discretization:}
\newline
\begin{eqnarray*}
\max_{\eta_i:\left[0,T\right]\rightarrow\left[0,1\right], i\in\mathcal{E}\setminus\mathcal{E}^{out}} &&\sum_{i\in\mathcal{E}}\sum_{m=1}^{n_T}\sum_{k=0}^{n_L^i} f(\rho_i(x_k,t_m))\Delta t \Delta x^i+\sum_{i\in\mathcal{E}\setminus\mathcal{E}^{out}}\sum_{m=1}^{n_T}\bar{f}_i(t_m)\cdot \eta^s_i(t_m)\Delta t\\&& +\sum_{j\in\mathcal{E}\setminus\mathcal{E}^{in}}\sum_{m=1}^{n_T}\bar{f}_j(t_m,\eta) \Delta t-\gamma\sum_{m=1}^{n_T}\sum_{i\in\mathcal{E}\setminus\mathcal{E}^{out}}\left(\eta^s_i(t_m)-\eta^s_i(t_{m-1})\right)
\end{eqnarray*}
s.t. \begin{eqnarray}
  \text{valid traffic light configurations}&  \eta^s\in\Omega \\
...
\rho_j^{n+1}=\rho_j^n-\frac{k}{h}\left[f(u^\star(\rho_j^n,\rho_{j+1}^n))-f(u^\star(\rho_{j-1}^n,\rho_{j}^n))\right]
 \\
  ...
  \text{junction influxes}&  \bar{f}_i=\min\lbrace \omega_i,c_i(M-\sum_{j\in\mathcal{O}}q_j)\rbrace \\
  \text{junction outfluxes}&\bar{f}_j=\begin{cases}
	\omega_j & q_j>0 \\
	\min\lbrace\omega_j, \sum_{i\in\mathcal{I}}\bar{f}_i\eta_i\bar{\Theta}_{ij}\rbrace & q_j=0

\end{cases}\\
	\text{queue}& q_j^{n+1}=q_j^n+dt \left(\sum_{i\in\mathcal{I}}\bar{\Theta}_{ij}\bar{f_i}\eta_i-\bar{f_j}\right)\\
  \text{inital densities, queue lengths, config}& \rho_{i}^0,q_{j}^0,\eta^0 \\
  \text{external inflow}&  \rho_i(0,t_m)\forall m=1...n_T \forall i \in \mathcal{E}^{in}
\end{eqnarray}
\newpage
...
Consider a constant $M>0$, describing the maximum capacity of the junction at any given time, and constants $c_i>0, i\in\mathcal{I}$ accounting for priorities given to the differnt incoming roads. \newline
We then require that the incoming fluxes $\bar{f}_i$ satisfy
\[\bar{f}_i=\min\lbrace \omega_i,c_i(M-\sum_{j\in\mathcal{O}}q_j), \hspace{1cm} i\in\mathcal{I}\]
Furthermore, the outgoing fluxes $\bar{f}_j$ should satisfy
